...
def _strip_launch_arg(raw: str, key: str) -> str:
    """Remove a single ROS launch ``key:=value`` override from a flat launch-args string."""
    kept: list[str] = []
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    for i in range(args.n):
        ...
        if not out_lines:
            ...
            failed_runs.append({"seed": seed, "rc": int(r.returncode), "reason": "missing_run_capture_output"})
            continue
        log_path = Path(out_lines[0].strip())
        if not log_path.is_file():
            print(f"[monte_carlo] log path missing: {log_path}", file=sys.stderr)
            failed_runs.append({"seed": seed, "rc": int(r.returncode), "reason": "missing_log_path"})
            continue
        result = analyze.parse_run_to_result(str(log_path))
        result["run_id"] = log_path.stem
        result["log_path"] = str(log_path)
        result["capture_rc"] = int(r.returncode)
    ...
    summary["n_collection_failures"] = len(failed_runs)
