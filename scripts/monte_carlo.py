...
def _launch_args_without_key(raw: str, key: str) -> str:
    """Drop caller-provided ``key:=...`` launch args so MC owns per-run seeds."""
    if not raw:
        return ""
    try:
    ...
    kept = [tok for tok in tokens if not (':=' in tok and tok.split(':=', 1)[0].strip() == key)]
    return " ".join(kept)
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    for i in range(args.n):
        ...
        if r.returncode not in (0, 124):
            ...
            collection_failures.append({"index": i, "seed": seed, "reason": "run_capture_rc", "returncode": r.returncode})
            continue
        out_lines = (r.stdout or "").strip().splitlines()
        if not out_lines:
            print("[monte_carlo] run produced no output; skipping", file=sys.stderr)
            collection_failures.append({"index": i, "seed": seed, "reason": "missing_stdout", "returncode": r.returncode})
            continue
        log_path = Path(out_lines[0].strip())
        if not log_path.is_file():
            print(f"[monte_carlo] log path missing: {log_path}", file=sys.stderr)
            collection_failures.append({"index": i, "seed": seed, "reason": "missing_log", "log_path": str(log_path)})
            continue
    ...
    summary["collected_runs"] = len(rows)
    summary["failed_run_count"] = len(collection_failures)
