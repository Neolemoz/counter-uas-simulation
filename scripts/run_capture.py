...
def _write_meta(path: Path, meta: RunMeta, *, capture_rc: int | None = None) -> None:
    meta_dict = asdict(meta)
    meta_dict["capture_rc"] = capture_rc
    path.write_text(json.dumps(meta_dict, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def run_capture(
    *,
    ...
    _write_meta(meta_path, meta, capture_rc=None)
    ...
    with log_path.open("w", encoding="utf-8") as f:
        ...
        try:
            ...
            if rc == 124:
                f.write("\n=== TIMEOUT ===\n")
                f.flush()
            _write_meta(meta_path, meta, capture_rc=rc)
            return log_path, meta_path, meta, rc
        except subprocess.TimeoutExpired:
            f.write("\n=== TIMEOUT ===\n")
            f.flush()
            _write_meta(meta_path, meta, capture_rc=124)
