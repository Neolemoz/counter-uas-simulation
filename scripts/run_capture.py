...
def _write_meta(meta_path: Path, meta: RunMeta, *, capture_rc: int | None = None) -> RunMeta:
    meta_out = replace(meta, capture_rc=capture_rc)
    meta_path.write_text(json.dumps(asdict(meta_out), indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return meta_out
    ...
    with log_path.open("w", encoding="utf-8") as f:
        ...
        try:
            ...
            if rc == 124:
                f.write("\n=== TIMEOUT ===\n")
                f.flush()
            return log_path, meta_path, _write_meta(meta_path, meta, capture_rc=rc), rc
        except subprocess.TimeoutExpired:
            f.write("\n=== TIMEOUT ===\n")
            f.flush()
            return log_path, meta_path, _write_meta(meta_path, meta, capture_rc=124), 124
