  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
        {SESSION_RUNTIME_PROFILE_OPTIONS.map((opt) => {
          ...
          return (
            <label
              key={opt.id}
              className={`flex min-w-[10rem] flex-1 cursor-pointer flex-col rounded border px-3 py-2 text-xs transition-colors ${
                checked
                  ? opt.id === "mock_adapter"
      ...
      <p className="text-[10px] text-slate-600">
        ...
        Live Gazebo and adapter maintainer commands are not available in the UI.
      </p>
    </fieldset>
  );
}
