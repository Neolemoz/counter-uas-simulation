...
def test_hit_with_capture_rc_124_is_not_failure() -> None:
    with tempfile.NamedTemporaryFile(mode='w', suffix='.log', delete=False, encoding='utf-8') as f:
...
def test_hit_no_specials_is_not_failure() -> None:
    text = (
...
def test_hit_with_timeout_marker_is_not_failure() -> None:
    text = '[HIT] interceptor_0  min_miss=0.5 m\n=== TIMEOUT ===\n'
    with tempfile.NamedTemporaryFile(mode='w', suffix='.log', delete=False, encoding='utf-8') as f:
        f.write(text)
        p = Path(f.name)
    try:
