...
Phase 6 contract freeze:
* ``guidance_lib.py`` owns CV intercept roots/tolerances and intercept points.
* ``guidance_kernel.py`` owns pairwise command semantics and memory transition.
* ``kinematic_plant.py`` owns command execution limits and rollout dynamics.
* ``interception_logic_node.py`` owns ROS adaptation, policy, logs, metrics, markers, and hit logic.
* Live measurement delay remains node-owned; kernel delay cases are covered here for pure callers.
* Live memory keys remain interceptor-id based in this phase.
* Offline timing parity through ``guidance_lib.py`` is not full command parity; command parity uses this kernel.
* Snapshot tolerances are intentionally tight and must not be loosened without a documented behavior change.
"""
