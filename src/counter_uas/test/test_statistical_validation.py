...
def test_paired_report_rejects_duplicate_seed_rows() -> None:
    layer_c = _load_layer_c()
    baseline = [
        {'success': 'true', 'miss_distance_m': '3.0', 'intercept_time_s': '9.0', 'seed': '1'},
...
def test_validate_manifest_requires_git_dirty_provenance_when_clean_required(tmp_path: Path) -> None:
    layer_c = _load_layer_c()
    log_path = tmp_path / 'run.log'
    log_path.write_text('', encoding='utf-8')
    log_path.with_suffix('.meta.json').write_text(json.dumps({'cohort': 'expected'}), encoding='utf-8')
    ...
    assert result['ok'] is False
    assert any('missing git_dirty provenance' in p for p in result['problems'])
    assert result['missing_git_dirty'] == [str(log_path)]
