        ...
        parameters=[
            {
                ...
                # PN: default off at far field; enable via use_pn_refinement + pn_blend_gain launch args.
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value='0.0',
                description=(
                    'Max |Δt_go_filt| per cycle after EMA (s); 0 disables slew cap on filtered t_go.'
                ),
            ),
            DeclareLaunchArgument(
                ...
                default_value='false',
                description=(
                    'If true, align_speed uses smoothed P_hit range; if false, raw P_hit (matches legacy).'
                ),
            ),
            DeclareLaunchArgument(
                ...
                default_value='0.0',
                description=(
                    'Scale PN blend toward zero when dist exceeds this (m); 0 = legacy full pn_blend at all ranges.'
                ),
            ),
            DeclareLaunchArgument(
                'use_pn_refinement',
                default_value='false',
                description='If true, apply proportional navigation correction to guidance direction.',
            ),
            DeclareLaunchArgument(
                'pn_blend_gain',
                default_value='0.0',
                description='PN mix-in gain before terminal scaling (0 = off).',
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                'noise_delay_mean_s',
                default_value='0.0',
                description='Optional added delay (seconds) for noisy_measurement_node publishes.',
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                ...
                default_value='false',
                description='If true, noisy_measurement_node may re-emit the last fresh sample during dropouts.',
            ),
            DeclareLaunchArgument(
                'stale_max_consecutive',
                default_value='0',
                description='Maximum consecutive stale detections to emit while stale_detection_enabled:=true.',
            ),
            DeclareLaunchArgument(
                'burst_dropout_prob',
                default_value='0.0',
