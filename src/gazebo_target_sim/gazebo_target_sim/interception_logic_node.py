...
class InterceptionLogicNode(Node):
    ...
    **Kinematics:** ``_compute_intercept`` / naive LOS use **x,y,z**; ``cmd_vel`` includes ``vz``.
    ...
    def __init__(self) -> None:
        ...
        self._guidance_target_source_kind = _meas_src or 'ground_truth'
    ...
    def _guidance_memory_key(self, *, selected: str, target_label: str = '') -> str:
        """Return the live guidance-memory key without changing legacy semantics.

        Single-target and current multi-target guidance both key memory by interceptor id.
        Keeping this as a helper makes the ownership explicit without introducing
        target/interceptor pair keys in this phase.
        """
        _ = target_label
        return selected

    def _target_kinematics_for_guidance(
        self,
        *,
        tx: float,
        ty: float,
        ...
        """Convert node target state to the source-agnostic kernel target contract.

        Point-like sources enter with velocity estimated by the node; tracks_state
        enters with the upstream filter velocity.  The node currently applies
        measurement delay before this adapter, so the kernel delay field remains zero.
        """
        _ = source_kind
        return TargetKinematics(
            position=(tx, ty, tz),
            velocity=(v_tx, v_ty, v_tz),
            measurement_delay_s=0.0,
        )

    def _interceptor_kinematics_for_guidance(
        self,
        *,
        ix: float,
        iy: float,
        ...
        """Convert node interceptor state to the kernel interceptor contract."""
        return InterceptorKinematics(
            position=(ix, iy, iz),
            velocity=(v_ix, v_iy, v_iz),
        )

    def _guidance_input_for_pair(
        self,
        *,
        memory_key: str,
        ...
        """Assemble the complete live-node adapter input for one pair."""
        return GuidanceInput(
            target=target,
            interceptor=interceptor,
            params=self._guidance_params_snapshot(),
            memory=self._guidance_memory_snapshot(memory_key),
    ...
    def _compute_guidance_command_for_pair(
        ...
        *,
        ...
        aim_z: float,
        target_label: str = '',
        target_source_kind: str = 'point',
    ) -> GuidanceCommand:
        memory_key = self._guidance_memory_key(selected=selected, target_label=target_label)
        target = self._target_kinematics_for_guidance(
            tx=tx,
            ty=ty,
            tz=tz,
            v_tx=v_tx,
        ...
        interceptor = self._interceptor_kinematics_for_guidance(
            ix=ix,
            iy=iy,
            iz=iz,
            v_ix=v_ix,
        ...
        self._store_guidance_memory(memory_key, cmd.memory_next, cmd.mode_transition)
        return cmd
    ...
    def _run_ram_guidance(
        ...
        *,
        ...
        guidance_cmd = self._compute_guidance_command_for_pair(
            ...
            aim_y=gy,
            aim_z=gz,
            target_source_kind=self._guidance_target_source_kind,
        )
    ...
    def _multi_ram_velocity_cmd(
        ...
        *,
        ...
        guidance_cmd_m = self._compute_guidance_command_for_pair(
            ...
            target_source_kind='multi_point',
        )
